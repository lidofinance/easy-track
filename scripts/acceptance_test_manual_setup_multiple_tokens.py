...
from brownie import interface, chain, network, AllowedRecipientsRegistry
...
deploy_config = deployment.AllowedRecipientsMultipleTokensManualSetupDeployConfig(
    tokens_registry="",
    limit=0,
    period=1,
    spent_amount=0,
...
def main():
    ...
    top_up_allowed_recipients = interface.TopUpAllowedRecipientsMultipleTokens(
        top_up_address
    )

    assert (
        top_up_allowed_recipients.allowedTokensRegistry()
