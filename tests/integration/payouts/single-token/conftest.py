...
def top_up_allowed_recipients_evm_script_factory(
    ...
    evm_script_factory = load_deployed_contract("TopUpAllowedRecipientsSingleToken")
    ...
    if evm_script_factory is None:
        ...
        evm_script_factory = TopUpAllowedRecipientsSingleToken.at(
...
def enact_top_up_allowed_recipient_motion_by_creation_tx(
    ...
    def _enact_top_up_allowed_recipient_motion_by_creation_tx(motion_creation_tx, spent_amount=0):
        top_up_allowed_recipients_evm_script_factory = TopUpAllowedRecipientsSingleToken.at(
...
@pytest.fixture(scope="module")
def allowed_recipients_factory(AllowedRecipientsFactorySingleToken, load_deployed_contract, deployer):

    loaded_allowed_recipients_factory = load_deployed_contract("AllowedRecipientsFactorySingleToken")

    if not loaded_allowed_recipients_factory is None:
        return loaded_allowed_recipients_factory

    return AllowedRecipientsFactorySingleToken.deploy({"from": deployer})
...
def allowed_recipients_builder(
    ...
    loaded_allowed_recipients_builder = load_deployed_contract("AllowedRecipientsBuilderSingleToken")

    if not loaded_allowed_recipients_builder is None:
        return loaded_allowed_recipients_builder

    return AllowedRecipientsBuilderSingleToken.deploy(
        allowed_recipients_factory,
