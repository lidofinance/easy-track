...
def test_set_date_time_contract(limits_checker):
    ...
    tx = limits_checker.setBokkyPooBahsDateTimeContract(new_address, {"from": set_parameters_role_holder})
    assert limits_checker.bokkyPooBahsDateTimeContract() == new_address
    assert_single_event(
        tx,
...
def test_fail_if_set_same_time_contract(limits_checker):
    ...
    with reverts("SAME_DATE_TIME_CONTRACT_ADDRESS"):
        limits_checker.setBokkyPooBahsDateTimeContract(current_address, {"from": set_parameters_role_holder})
...
def test_unsafe_set_spent_amount_when_spent_amount_exceeds_limit(limits_checker):
    ...
    limits_checker.setLimitParameters(initial_limit, initial_duration, {"from": set_parameters_role_holder})
    ...
    with reverts("ERROR_SPENT_AMOUNT_EXCEEDS_LIMIT"):
        limits_checker.unsafeSetSpentAmount(initial_limit + 1, {"from": set_parameters_role_holder})
...
def test_unsafe_set_spent_amount_when_new_spent_amount_the_same(limits_checker):
    ...
    tx = limits_checker.unsafeSetSpentAmount(already_spent_amount, {"from": set_parameters_role_holder})

    assert limits_checker.getPeriodState()["_alreadySpentAmount"] == already_spent_amount
...
def test_unsafe_set_spent_amount(limits_checker):
    ...
    tx = limits_checker.unsafeSetSpentAmount(new_spent_amount, {"from": set_parameters_role_holder})
...
def test_period_range_calculation_for_all_allowed_period_durations(
    ...
    for _ in range(24):
        for duration in [1, 2, 3, 6, 12]:
            limits_checker.setLimitParameters(period_limit, duration, {"from": set_parameters_role_holder})
...
def test_fail_if_set_incorrect_period_durations(limits_checker, period_duration):
    ...
    with reverts("INVALID_PERIOD_DURATION"):
        limits_checker.setLimitParameters(period_limit, period_duration, {"from": set_parameters_role_holder})
...
def test_fail_if_set_limit_greater_than_max_limit(limits_checker):
    ...
    limits_checker.setLimitParameters(period_limit - 1, period_duration, {"from": set_parameters_role_holder})
...
def test_update_spent_amount_within_the_limit(limits_checker):
    ...
    assert limits_checker.isUnderSpendableBalance(period_limit, period_duration * MAX_SECONDS_IN_MONTH)
    assert_single_event(
        tx,
...
def test_update_spent_amount_precisely_to_the_limit_in_multiple_portions(
    ...
    assert limits_checker.getPeriodState()["_spendableBalanceInPeriod"] == period_limit - 2 * spending
...
def test_spendable_amount_increased_if_limit_increased(limits_checker):
    ...
    limits_checker.setLimitParameters(new_period_limit, period_duration, {"from": set_parameters_role_holder})
    assert limits_checker.getPeriodState()["_alreadySpentAmount"] == spending
...
def test_spendable_amount_renewal_if_period_duration_changed(
    ...
    limits_checker.setLimitParameters(period_limit, initial_period_duration, {"from": set_parameters_role_holder})
    advance_chain_time_to_beginning_of_the_next_period(max(initial_period_duration, new_period_duration))
    ...
    limits_checker.setLimitParameters(period_limit, new_period_duration, {"from": set_parameters_role_holder})
    with reverts("SUM_EXCEEDS_SPENDABLE_BALANCE"):
        limits_checker.updateSpentAmount(1, {"from": update_spent_amount_role_holder})
...
def test_period_start_from_timestamp(
    ...
    assert limits_checker.getPeriodStartFromTimestamp(inputs[0] - 3600) < expected_result  # 1 hour before the period
    assert limits_checker.getPeriodStartFromTimestamp(inputs[0] - 1) < expected_result  # second before the period
    assert (
...
def test_period_end_from_timestamp(
    ...
    assert limits_checker.getPeriodEndFromTimestamp(inputs[0] - 3600) < expected_result  # hour before the period
    assert limits_checker.getPeriodEndFromTimestamp(inputs[0] - 1) < expected_result  # second before the period
    assert (
