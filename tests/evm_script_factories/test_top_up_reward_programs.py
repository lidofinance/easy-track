...
def test_deploy(owner, reward_programs_registry, finance, ldo, TopUpRewardPrograms):
    ...
    contract = owner.deploy(TopUpRewardPrograms, owner, reward_programs_registry, finance, ldo)
...
def test_create_evm_script_called_by_stranger(stranger, top_up_reward_programs):
    ...
    with reverts("CALLER_IS_FORBIDDEN"):
        top_up_reward_programs.createEVMScript(stranger, encode_call_data([], []), {"from": stranger})
...
def test_create_evm_script_zero_amount(
    ...
    amounts = [1**18, 0]
    reward_programs_registry.addRewardProgram(REWARD_PROGRAM_ADDRESSES[0], "", {"from": evm_script_executor_stub})
    reward_programs_registry.addRewardProgram(REWARD_PROGRAM_ADDRESSES[1], "", {"from": evm_script_executor_stub})
    with reverts("ZERO_AMOUNT"):
        top_up_reward_programs.createEVMScript(owner, encode_call_data(REWARD_PROGRAM_ADDRESSES, amounts))
