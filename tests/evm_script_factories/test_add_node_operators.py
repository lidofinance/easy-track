...
from brownie import reverts, AddNodeOperators, web3, convert, ZERO_ADDRESS
...
def test_create_evm_script_called_by_stranger(stranger, add_node_operators_factory):
    ...
    with reverts("CALLER_IS_FORBIDDEN"):
        add_node_operators_factory.createEVMScript(stranger, EVM_SCRIPT_CALL_DATA)
...
def test_manager_already_has_permission_for_node_operator(
    ...
    "Must revert with message 'MANAGER_ALREADY_HAS_ROLE' when manager has MANAGE_SIGNING_KEYS role with paramer"
    no_count = node_operators_registry.getNodeOperatorsCount()
...
def test_lido_reward_address(
    owner, add_node_operators_factory, node_operators_registry, steth
):
...
def test_create_evm_script(
    owner, add_node_operators_factory, node_operators_registry, acl
):
    ...
    evm_script = add_node_operators_factory.createEVMScript(owner, CALL_DATA)
    ...
    for id, input_param in enumerate(input_params):
        scripts.append(
            (
                ...
                node_operators_registry.addNodeOperator.encode_input(
        ...
        scripts.append(
            (
                ...
                acl.grantPermissionP.encode_input(
                    ...
                    [
                        convert.to_uint(
                            (0 << 248) + (1 << 240) + no_count + id, "uint256"
                        )
                    ],
                ),
            )
...
def test_decode_evm_script_call_data(
    node_operators_registry, add_node_operators_factory
):
    ...
    assert (
        add_node_operators_factory.decodeEVMScriptCallData(EVM_SCRIPT_CALL_DATA)
        == input_params
    )
