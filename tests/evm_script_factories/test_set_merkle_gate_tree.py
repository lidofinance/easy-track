import pytest
from brownie import reverts, SetMerkleGateTree, MerkleGateStub, ZERO_ADDRESS # type: ignore
...
@pytest.fixture(scope="module") 
def set_merkle_gate_tree_factory(owner, merkle_gate_stub):
    return SetMerkleGateTree.deploy(owner, "IdentifiedCommunityStakerSetTreeParams", merkle_gate_stub, {"from": owner})


def test_deploy(owner, merkle_gate_stub, set_merkle_gate_tree_factory):
    """Must deploy contract with correct data"""
    assert set_merkle_gate_tree_factory.trustedCaller() == owner
    assert set_merkle_gate_tree_factory.merkleGate() == merkle_gate_stub
    assert set_merkle_gate_tree_factory.name() == "IdentifiedCommunityStakerSetTreeParams"
...
def test_empty_tree_root(owner, set_merkle_gate_tree_factory):
...
def test_empty_tree_cid(owner, set_merkle_gate_tree_factory):
...
def test_decode_evm_script_call_data(set_merkle_gate_tree_factory):
    """Must decode EVMScript call data correctly"""
    tree_root = bytes.fromhex("1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")
    tree_cid = "QmTest123456789"
    
    EVM_SCRIPT_CALLDATA = create_calldata(tree_root, tree_cid)
    decoded_root, decoded_cid = set_merkle_gate_tree_factory.decodeEVMScriptCallData(EVM_SCRIPT_CALLDATA)
