...
def test_deploy(owner, cs_module, csm_settle_general_delayed_penalty_factory):
...
def test_create_evm_script_called_by_stranger(stranger, csm_settle_general_delayed_penalty_factory):
    ...
    with reverts("CALLER_IS_FORBIDDEN"):
        csm_settle_general_delayed_penalty_factory.createEVMScript(stranger, EVM_SCRIPT_CALLDATA)


def test_empty_calldata(owner, csm_settle_general_delayed_penalty_factory):
    EMPTY_CALLDATA = create_calldata([], [])
    with reverts('EMPTY_NODE_OPERATORS_IDS'):
        csm_settle_general_delayed_penalty_factory.createEVMScript(owner, EMPTY_CALLDATA)


def test_operator_id_out_of_range(owner, csm_settle_general_delayed_penalty_factory, cs_module):
    ...
    with reverts('OUT_OF_RANGE_NODE_OPERATOR_ID'):
        csm_settle_general_delayed_penalty_factory.createEVMScript(owner, CALLDATA)


def test_create_evm_script(owner, csm_settle_general_delayed_penalty_factory, cs_module, fill_cs_module):
    "Must create correct EVMScript if all requirements are met"
    node_operator_ids = [0]
    max_amounts = [33 * 10 ** 18]

    EVM_SCRIPT_CALLDATA = create_calldata(node_operator_ids, max_amounts)
    evm_script = csm_settle_general_delayed_penalty_factory.createEVMScript(owner, EVM_SCRIPT_CALLDATA)
...
def test_decode_evm_script_call_data(csm_settle_general_delayed_penalty_factory):
    "Must decode EVMScript call data correctly"
    node_operator_ids = [0, 1, 2]
    max_amounts = [1000, 2000, 3000]

    EVM_SCRIPT_CALLDATA = create_calldata(node_operator_ids, max_amounts)
    decoded_ids, decoded_amounts = csm_settle_general_delayed_penalty_factory.decodeEVMScriptCallData(EVM_SCRIPT_CALLDATA)
    assert decoded_ids == node_operator_ids
    assert decoded_amounts == max_amounts


def test_node_operators_ids_and_max_amounts_length_mismatch(owner, csm_settle_general_delayed_penalty_factory):
...
def test_max_amount_should_be_greater_than_zero(owner, csm_settle_general_delayed_penalty_factory, fill_cs_module):
...
def test_max_amount_should_be_greater_than_actual_locked(owner, csm_settle_general_delayed_penalty_factory, cs_module, fill_cs_module):
