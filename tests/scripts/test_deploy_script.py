...
def test_deploy_script(accounts):
    ...
    assert increase_node_operators_staking_limit.nodeOperatorsRegistry() == lido_contracts.node_operators_registry
    ...
    assert reward_programs_registry.hasRole(reward_programs_registry.DEFAULT_ADMIN_ROLE(), lido_contracts.aragon.voting)
    assert reward_programs_registry.hasRole(reward_programs_registry.ADD_REWARD_PROGRAM_ROLE(), evm_script_executor)
    assert reward_programs_registry.hasRole(
        reward_programs_registry.ADD_REWARD_PROGRAM_ROLE(), lido_contracts.aragon.voting
    )
    assert reward_programs_registry.hasRole(reward_programs_registry.REMOVE_REWARD_PROGRAM_ROLE(), evm_script_executor)
    assert reward_programs_registry.hasRole(
    ...
    set_node_operator_staking_limit_permission = (
        ...
        + lido_contracts.node_operators_registry.setNodeOperatorStakingLimit.signature[2:]
    )
    new_immediate_payment_permission = (
        lido_contracts.aragon.finance.address + lido_contracts.aragon.finance.newImmediatePayment.signature[2:]
    )
    ...
    assert easy_track.evmScriptFactoryPermissions(top_up_lego_program) == new_immediate_payment_permission
    assert easy_track.evmScriptFactoryPermissions(add_reward_program) == add_reward_program_permission
    assert easy_track.evmScriptFactoryPermissions(remove_reward_program) == remove_reward_program_permission
    assert easy_track.evmScriptFactoryPermissions(top_up_reward_programs) == new_immediate_payment_permission
