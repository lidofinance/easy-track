...
def test_add_evm_script_factory_called_without_permissions(
    ...
    with reverts(access_revert_message(stranger)):
        evm_script_factories_registry.addEVMScriptFactory(stranger, permissions, {"from": stranger})


def test_add_evm_script_factory_empty_permissions(owner, stranger, evm_script_factories_registry):
    "Must revert with message 'INVALID_PERMISSIONS' if called with empty permissions"
    with reverts("INVALID_PERMISSIONS"):
        evm_script_factories_registry.addEVMScriptFactory(stranger, "0x", {"from": owner})


def test_add_evm_script_factory_invalid_length(owner, stranger, evm_script_factories_registry):
    "Must revert with message 'INVALID_PERMISSIONS' if called with permissions which have incorrect length"
    with reverts("INVALID_PERMISSIONS"):
        evm_script_factories_registry.addEVMScriptFactory(stranger, "0x0011223344", {"from": owner})
...
def test_add_evm_script(owner, stranger, evm_script_factories_registry):
    ...
    permissions = stranger.address + "ffccddee"
    tx = evm_script_factories_registry.addEVMScriptFactory(stranger, permissions, {"from": owner})
...
def test_add_evm_script_twice(owner, stranger, evm_script_factories_registry):
    ...
    permissions = stranger.address + "ffccddee"
    evm_script_factories_registry.addEVMScriptFactory(stranger, permissions, {"from": owner})
    with reverts("EVM_SCRIPT_FACTORY_ALREADY_ADDED"):
        evm_script_factories_registry.addEVMScriptFactory(stranger, permissions, {"from": owner})


def test_remove_evm_script_factory_not_found(owner, stranger, evm_script_factories_registry):
...
def test_remove_evm_script_factory(owner, stranger, evm_script_factories_registry, extra_evm_script_factories):
    ...
    for evm_script_factory in extra_evm_script_factories:
        evm_script_factories_registry.addEVMScriptFactory(evm_script_factory, permissions, {"from": owner})
    ...
    for index in removing_order:
        ...
        assert evm_script_factories_registry.isEVMScriptFactory(evm_script_factory_to_remove)
        tx = evm_script_factories_registry.removeEVMScriptFactory(evm_script_factory_to_remove, {"from": owner})
        assert not evm_script_factories_registry.isEVMScriptFactory(evm_script_factory_to_remove)

        # validate events
        assert len(tx.events) == 1
        assert tx.events["EVMScriptFactoryRemoved"]["_evmScriptFactory"] == evm_script_factory_to_remove

        # validate that was deleted correct address by join
        # test set with resulting set their size must be same
        evm_script_factories_after_remove = evm_script_factories_registry.getEVMScriptFactories()
        assert len(evm_script_factories) == len(evm_script_factories_after_remove)

        len(set(evm_script_factories).union(evm_script_factories_after_remove)) == len(evm_script_factories)
