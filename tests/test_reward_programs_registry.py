...
def test_deploy(owner, voting, evm_script_executor_stub, RewardProgramsRegistry):
    ...
    assert contract.hasRole(contract.ADD_REWARD_PROGRAM_ROLE(), evm_script_executor_stub)
    assert contract.hasRole(contract.REMOVE_REWARD_PROGRAM_ROLE(), evm_script_executor_stub)
...
def test_add_reward_program_called_by_stranger(stranger, reward_programs_registry):
    ...
    assert not reward_programs_registry.hasRole(reward_programs_registry.ADD_REWARD_PROGRAM_ROLE(), stranger)
    with reverts(access_revert_message(stranger, reward_programs_registry.ADD_REWARD_PROGRAM_ROLE())):
...
def test_remove_reward_program_called_by_stranger(owner, stranger, reward_programs_registry):
    ...
    assert not reward_programs_registry.hasRole(reward_programs_registry.REMOVE_REWARD_PROGRAM_ROLE(), stranger)
    with reverts(access_revert_message(stranger, reward_programs_registry.REMOVE_REWARD_PROGRAM_ROLE())):
...
def test_remove_reward_program_with_not_existed_reward_program(
    ...
    with reverts("REWARD_PROGRAM_NOT_FOUND"):
        reward_programs_registry.removeRewardProgram(stranger, {"from": evm_script_executor_stub})


def test_remove_reward_program(accounts, reward_programs_registry, evm_script_executor_stub):
    ...
    for reward_program in reward_programs:
        reward_programs_registry.addRewardProgram(reward_program, "", {"from": evm_script_executor_stub})
    ...
    for index in removing_order:
        ...
        tx = reward_programs_registry.removeRewardProgram(reward_program, {"from": evm_script_executor_stub})
        # validate events
        assert len(tx.events) == 1
        ...
        assert len(set(reward_programs).union(contract_reward_programs)) == len(contract_reward_programs)
