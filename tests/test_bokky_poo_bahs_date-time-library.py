...
def test_property_based_timestamp_to_date(date_time_library, progress, timestamp):
    progress_counter = progress.get_or_create_instance(
        "test_property_based_timestamp_to_date", max_value=5000
    )
...
def test_timestamp_from_date_automated(date_time_library, start_date, end_date):
    ...
    while datetime.timestamp(current_date) < get_timestamp_from_date(*end_date):
        assert (
            date_time_library.timestampFromDate(
                current_date.year,
                current_date.month,
                current_date.day,
            )
...
@given(
    days_shift=strategy(
        "int", min_value=0, max_value=(TEST_END_DATE - TEST_START_DATE).days
    ),
)
@settings(max_examples=5000, verbosity=Verbosity.quiet)
def test_property_based_timestamp_from_date(date_time_library, progress, days_shift):
    progress_counter = progress.get_or_create_instance(
        "test_property_based_timestamp_from_date", max_value=5000
    )
...
@pytest.mark.parametrize(
    ...
    + [
        ((2020, 1, 31), months) for months in [1, 2, 3, 6, 8, 12, 24]
    ],  # Last day of month
)
