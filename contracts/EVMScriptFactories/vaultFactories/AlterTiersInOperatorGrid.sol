    ...
    function _validateInputData(uint256[] memory _tierIds, TierParams[] memory _tierParams) private view {
        ...
        for (uint256 i = 0; i < _tierIds.length; i++) {
            ...
            if (_tierIds[i] == DEFAULT_TIER_ID) {
                ...
                require(_tierParams[i].shareLimit <= MAX_SHARE_LIMIT, ERROR_TIER_SHARE_LIMIT_TOO_HIGH);
            }
            ...
            require(_tierParams[i].forcedRebalanceThresholdBP + 10 < _tierParams[i].reserveRatioBP, ERROR_FORCED_REBALANCE_THRESHOLD_TOO_HIGH);
