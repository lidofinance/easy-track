...
async function renameOpenzeppelinDependencies() {
    ...
    const remapping = {
        '@openzeppelin/contracts': 'OpenZeppelin/openzeppelin-contracts@4.1.0/contracts',
        '@openzeppelin/contracts-upgradeable': 'OpenZeppelin/openzeppelin-contracts-upgradeable@4.1.0/contracts'
    }
    ...
    for (const localPath of Object.keys(remapping)) {
        ...
        try {
            ...
            const res = await copy(from, to, { overwrite: true, expand: true, dot: true, junk: true })
        } catch (error) {
            console.error(error)
            process.exit(1)
        }
